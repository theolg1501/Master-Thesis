...
def executeFlightPlanTheo(waypoints_json):
    ...
    cmds.add(Command(0, 0, 0, mavutil.mavlink.MAV_FRAME_GLOBAL_RELATIVE_ALT, mavutil.mavlink.MAV_CMD_NAV_WAYPOINT, 0, 0, 0, 0, 0, 0, originPoint.lat, originPoint.lon, altitude))
    ...
    while True:
        ...
        if nextwaypoint == len(waypoints) + 1:  # Dummy waypoint - as soon as we reach waypoint 4 this is true and we exit.
...
def AutopilotService (connection_mode, operation_mode, external_broker, username, password):
    ...
    internal_broker_address = "localhost"  # '10.10.10.1'
    internal_broker_port = 1884

    if connection_mode == 'global':
        external_broker_address = external_broker
    else:
        external_broker_address = 'localhost'  # '10.10.10.1'
