...
These are the different values for the state of the autopilot:
    'connected' (only when connected the telemetry_info packet will be sent every 250 milliseconds)
    'arming'
    'armed'
    'disarmed'
    'takingOff'
...
def send_telemetry_info():
    ...
    while sending_telemetry_info:
        ...
        internal_client.publish(sending_topic + "/telemetryInfo", json.dumps(get_telemetry_info()))
        time.sleep(0.25)
...
def process_message(message, client):
    ...
    # message from other components, format:"XXX/autopilotService/YYY"
    ...
    if command == "connect":
        if state == 'disconnected':
            ...
            if op_mode == 'simulation':
                connection_string = "tcp:10.10.10.201:5763"
    ...
    if command == 'executeFlightPlan':
        ...
        with open(waypoints_json_dir, 'r') as j:
            w = threading.Thread(target=executeFlightPlan2, args=[j, ])
            w.start()
...
if __name__ == '__main__':
    connection_mode = sys.argv[1]  # global or local, in our project there is only external broker
